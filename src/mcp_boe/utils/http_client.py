...
class BOEHTTPClient:
    ...
    def build_search_query(
        ...
        if text:
            # Busca en título Y texto completo, priorizando el título.
            # - Si el texto tiene varias palabras, se busca como FRASE EXACTA
            #   ("sector público") para no esparcir el match a cualquier norma
            #   que contenga "sector" O "público" por separado.
            # - Se aplica boost (^3) al título para que las normas cuyo TÍTULO
            #   coincide (p. ej. "Ley 40/2015 ... del Sector Público") suban a
            #   los primeros resultados en vez de perderse entre matches de texto.
            terms = text.strip()
            escaped = terms.replace('"', '\\"')
            if " " in terms:
                phrase = f'"{escaped}"'
            else:
                phrase = escaped
            query_parts.append(f'(titulo:({phrase})^3 OR texto:({phrase}))')
        if title:
